...
def good_card(computer_hand,card):
    """
    Checks if card is a good card to be replaced with one of cards in hand
    Parameter: computers hand; card
    Returns index of card to be replaced; or False if the card is not considered good
    """
    sum=0
    for i in range (len(computer_hand)):
        if card > computer_hand[i]:
            sum=+1
    if sum == (len(computer_hand)):
        return int(len(computer_hand))
    sum2=0
    ...
    if sum2 == (len(computer_hand)):
        return 0

    for i in range (len(computer_hand)):
        if card > computer_hand[i] and card < computer_hand[i+1]:
            return (len(computer_hand))//2
...
def computer_play(computer_hand, deck, discard):
    ...
    card= good_card(computer_hand, discard_card)
    if card != False:
        ...
        number_of_card = card
        print("Computer: Replacing it with  " + str(computer_hand[card]))

        #Modify the discard pile and the computer's hand
        find_and_replace(discard_card,computer_hand[card],computer_hand,discard)
        ...
        card2= good_card(computer_hand,deck_card)
        #coin = random.random()
        if card2 != False:
        #Randomly decide whether to keep the deck card or not
            ...
            print("Computer: Replacing it with " + str(computer_hand[card2]))

            #Modify the discard pile and the computer's hand
            find_and_replace(deck_card,computer_hand[card2],computer_hand,discard)
