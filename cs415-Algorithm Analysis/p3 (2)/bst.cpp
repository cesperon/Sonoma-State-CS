...
//Inorder traverses the tree and returns a vector of all the keys
//in order
vector<string> BST::returnIndex() {
	vector<string> keys;
	keys.resize(0);
	returnIndexHelper(root, keys);
	return keys;
}

void BST::returnIndexHelper(node *t, vector<string> &keys) {
	if (t == NULL) {
		return;
	}
	returnIndexHelper(t->left, keys);
	keys.push_back(t->key);
	returnIndexHelper(t->right, keys);

}

double BST::searchIndex(vector<string> keys) {
	bool contains = false;
...
void BST::buildTree(ifstream & input, bool printmsg){
